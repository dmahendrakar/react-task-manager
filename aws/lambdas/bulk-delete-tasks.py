...
def lambda_handler(event, context):
    ...
    queue = sqs.get_queue_by_name(QueueName='bulk-delete-tasks')
    
    try:
        for task in json.loads(event['body']):
            logger.info('Adding task to delete sqs, ' + json.dumps(task))
            response = queue.send_message(MessageBody=json.dumps(task))
            
            logger.info("DeleteItem sqs enqueue succeeded:");
        ...
        return generate_response(500, 'Error occurred while processing bulk delete tasks')
